    ...
    <div class="d-flex align-items-stretch" id="kt_header_nav">
        ...
        <div class="header-menu align-items-stretch" data-kt-drawer="true" data-kt-drawer-name="header-menu" data-kt-drawer-activate="{default: true, lg: false}" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'200px', '300px': '250px'}" data-kt-drawer-direction="end" data-kt-drawer-toggle="#kt_header_menu_mobile_toggle" data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_body', lg: '#kt_header_nav'}">
            ...
            <div class="menu menu-lg-rounded menu-column menu-lg-row menu-state-bg menu-title-gray-700 menu-state-title-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-400 fw-bold my-5 my-lg-0 align-items-stretch" id="#kt_header_menu" data-kt-menu="true">
                <span class="title-heading">questions & options</span>
...
<script>
        ...
        for(var i in rs) {
            ...
            for(var k in options){
                ...
                for(var h in option_hyps){
                    ...
                    if(option_hyps[h].option_hyp_point == null){
                        $('#hypo_box_' + rs[i].question_id + '_' + k + '_' + option_hyps[h].hyp_id).children('input').eq(1).addClass('option-content-alert');
                    }
                }
            }
    ...
    */
    ...
    $(document).on('click', '.remove-question', function(e){
        ...
        $question_content = $(e.target).parent().find('input').val();
    ...
    $(document).on('click','.add-option', function(e){
        ...
        $('#option_box_' + $new_question_id + '_' + new_option_id).find('input').focus(); 
        // set focus on new added option textbox

        $.post('/getdatainjson/add_empty_option_hyps', {option_id : new_option_id}, function(rs){
        ...
        $.post('/getdatainjson/hyps', function(result){
            ...
            for(var h in rs){
                $('#option_box_'+ $new_question_id + '_' + new_option_id).append(
                    new_hyp.compose({
                        'option_id'  : new_option_id,
                        'hyp_id' : rs[h].hyp_id,
                ...
                $('#hypo_box_'  + $new_question_id + '_' + new_option_id + '_' + rs[h].hyp_id).children('input').eq(1).addClass('option-content-alert');
    ...
    $(document).on('click', '.remove-option', function(e){
        ...
        $remove_option_id = $(e.target).parent().attr('option_id');
        if(!window.confirm("Do you really want to delete this option?")) return;
            ...
            $.post('/getdatainjson/add_option_hyp',{
                ...
                if(status == 'success'){
                   setTimeout(function() {
                          $(e.target).removeClass('input_ajax_started'); 
                          // ajax waiting effect ended after 1s
                        }, 1000); // ajax waiting effect ended.
                }
            });
    ...
    $('.scopes-select').change(function(e){
        ...
        $.post('/getdatainjson/allquestions', function(result){
            ...
            var rs = JSON.parse(result);
            var hyp_list = rs[0][0];
            if($scope_id != $default_scope_id){
                rs = rs.filter(node => node.scope_id == $scope_id);
            }
             ...
             */
            for(var i in rs) {
                if((i == 0) &&($scope_id == $default_scope_id)){
                    continue;
                }
