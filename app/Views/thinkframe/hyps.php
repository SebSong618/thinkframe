...
<script src="assets/js/custom/custom-thinkframe-1-5.js"></script>

<link rel="stylesheet" href="assets/css/custom-thinkframe-1-5.css">
...
<script>
    ...
    $.post('/getdatainjson/getMaxIds', function(result){
        ...
        option_hyp_last_id  = rs.option_hyp_last_id;
        new_option_hyp_id = option_hyp_last_id + 1;
        ...
        $.post('/getdatainjson/allhyps', {scope_id : $('.scopes-select').val() }, function(result){
            ...
            for(var i in rs) {
                ...
                $('#hyp_block_' + rs[i].hyp_id).append(
                    scale_labels.compose({
                        'label_m_100' : rs[i].label_m_100,
                        'label_m_50'  : rs[i].label_m_50,
                        'label_0'     : rs[i].label_0,
                        'label_p_50'  : rs[i].label_p_50,
                        'label_p_100' : rs[i].label_p_100,
                        'hyp_id' : rs[i].hyp_id,
                    })
                );
                    ...
                    for(var n in options){
                        ...
                        $('#hyp_block_' + rs[i].hyp_id).append(
                            option_in_hyp.compose({
                                ...
                                'range_show' : options[n].strength == null ? 'hidden' : 'visible',
                                'strength' : options[n].strength == null ? -1 : options[n].strength
                            })
                        );

                        if(options[n].strength == null) {
                            $('#strength_link_' + options[n].option_hyp_id).val(-1);
                            $('#strength_link_' + options[n].option_hyp_id).prev().prev().css('visibility','hidden');
                        }
                        else{
                            $('#strength_link_' + options[n].option_hyp_id).val(options[n].strength);
                        }

                        if(options[n].strength == -1 || options[n].strength == 1000){
                            $('#strength_link_' + options[n].option_hyp_id).prev().prev().css('visibility','hidden');
                        }

                        if(parseInt(options[n].strength) == 1000){ //NONE relation
                        ...
                        if(options[n].strength == null || options[n].strength == -1){
                            $('#'+options[n].option_hyp_id).parent().children('input').eq(2).addClass('option-content-alert');
                        }
    ...
    $(document).on('change', '.strength-list', function(e){
        ...
        if(strength == -1 || strength == 1000){ // This means "?"
            $(e.target).prev().removeClass().addClass('hyp-option-range slider-1000');
            $(e.target).prev().prev().css('visibility','hidden');
        }
        else{
            $(e.target).prev().removeClass().addClass('hyp-option-range slider-' + strength);
            $(e.target).prev().css('visibility','visible');
            $(e.target).prev().prev().css('visibility','visible'); 
        }

        $option_hyp_id = $(e.target).parent().children('input').eq(0).attr('id');
        $hyp_id = $(e.target).parent().parent().attr('hyp_id');

        $.post('/getdatainjson/update_strength',{
    ...
    $('#bt_all_zero').click(function(e){
        ...
        $(".option-point-hyp").each(function (index, element) {
            if($(this).val() == ""){
                $(this).next().next().val(1000); // set to NONE
                $(this).next().next().next().next().removeClass('option-content-alert');
        ...
        $.post('/getdatainjson/set_zeros_on_blanks', {table_selection : "option_hyp", option_hyps: JSON.stringify($option_hyps)}, function(rs){
        ...
        $.post('/getdatainjson/allquestions_', {scope_id : $scope_id},function(result){
            ...
            for(var i in rs) {
                ...
                for(var k in options){
                     ...
                     */ 
                    var $option_hyp_id = new_option_hyp_id++;
     ...
     * label_m_100
     */
    $(document).on('change', '.label_m_100', function(e){
        var hyp_id = $(e.target).parent().attr('hyp_id');

        $.post('/getdatainjson/update_label_m_100',{
            hyp_id : hyp_id,
            label_m_100 : $(e.target).val() 
            }, function(res,status){
                if(status == 'success'){
     ...
     * label_m_50
     */
    $(document).on('change', '.label_m_50', function(e){
        var hyp_id = $(e.target).parent().attr('hyp_id');

        $.post('/getdatainjson/update_label_m_50',{
            hyp_id : hyp_id,
            label_m_50 : $(e.target).val() 
            }, function(res,status){
                if(status == 'success'){
                    $(e.target).removeClass('input_ajax_started').addClass('green_tick_started');
                    setTimeout(function() {
     ...
     * label_0
     */
    $(document).on('change', '.label_0', function(e){
       var hyp_id = $(e.target).parent().attr('hyp_id');

        $.post('/getdatainjson/update_label_0',{
            hyp_id : hyp_id,
            label_0 : $(e.target).val() 
            }, function(res,status){
                if(status == 'success'){
                    $(e.target).removeClass('input_ajax_started').addClass('green_tick_started');
                    setTimeout(function() {
     ...
     * label_p_50
     */
    $(document).on('change', '.label_p_50', function(e){
        var hyp_id = $(e.target).parent().attr('hyp_id');

        $.post('/getdatainjson/update_label_p_50',{
            hyp_id : hyp_id,
            label_p_50 : $(e.target).val() 
            }, function(res,status){
                if(status == 'success'){
                    $(e.target).removeClass('input_ajax_started').addClass('green_tick_started');
                    setTimeout(function() {
     ...
     * label_p_100
     */
    $(document).on('change', '.label_p_100', function(e){
        var hyp_id = $(e.target).parent().attr('hyp_id');

        $.post('/getdatainjson/update_label_p_100',{
            hyp_id : hyp_id,
            label_p_100 : $(e.target).val() 
            }, function(res,status){
                if(status == 'success'){
                    $(e.target).removeClass('input_ajax_started').addClass('green_tick_started');
                    setTimeout(function() {
