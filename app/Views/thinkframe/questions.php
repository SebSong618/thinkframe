    ...
    <div class="d-flex align-items-stretch" id="kt_header_nav">
        ...
        <div class="header-menu align-items-stretch" data-kt-drawer="true" data-kt-drawer-name="header-menu" data-kt-drawer-activate="{default: true, lg: false}" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'200px', '300px': '250px'}" data-kt-drawer-direction="end" data-kt-drawer-toggle="#kt_header_menu_mobile_toggle" data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_body', lg: '#kt_header_nav'}">
            ...
            <div class="menu menu-lg-rounded menu-column menu-lg-row menu-state-bg menu-title-gray-700 menu-state-title-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-400 fw-bold my-5 my-lg-0 align-items-stretch" id="#kt_header_menu" data-kt-menu="true">
                <span class="title-heading question-title">questions & options</span>
                ...
                <div class="menu-item me-lg-1">
                 ...
                 <div class="menu-item me-lg-1">
                    <button type="button" id="bt_change_order" class="btn btn-lg btn-primary">
                        &nbsp;CHANGE ORDER 
                    </button>
                </div>
    ...
    <div id="kt_content_container" class="container">
        ...
        <div class="card question-page">
            ...
            <div class="card-body pt-0">
                ...
                <div class="modal fade" id="option_confirm_delete" tabindex="-1" aria-hidden="true">
                    <!--begin::Modal dialog-->
                    <div class="modal-dialog modal-dialog-centered">
                        <!--begin::Modal content-->
                        <div class="modal-content">
                            ...
                            <div class="modal-body scroll-y">
                                ...
                                <div class="mx-auto">
                                    ...
                                    <div class="text-center">
                                        <!--begin::Title-->
                                        <h1 class="option-delete-question">Do you really want to delete this combo?</h1>
                                        <!--end::Title-->
                                    </div>
...
<script src="assets/js/custom/jquery-ui.min.js"></script>
...
<script>
    ...
    var is_order_button_clicked = true;
    ...
    $.post('/getdatainjson/getMaxIds', function(result){
        ...
        // get the max question_id, option_id, link id on page loading.
        ...
        $.post('/getdatainjson/allquestions', {scope_id : $('.scopes-select').val()}, function(result){
            ...
            var all_hyps = rs[0][0];
            /**
             * Hyps data are included in rs, so to list all questions, we should skip hyps.
             */
            for(var i in rs) {
                if(i == 0){
                    continue;
                ...
                for(var k in options){
                    $('#question_block_' + rs[i].question_id).append(
                        new_option.compose({
                            'question_id' : rs[i].question_id,
                            'option_id'  : options[k].option_id,
                    ...
                    for(var h in option_hyps){
                        if(option_hyps[h].scope_id == $('.scopes-select').val()){

                            var hyp = all_hyps.filter(entry => entry.hyp_id == option_hyps[h].hyp_id);
                   
                            $('#option_box_'+rs[i].question_id+'_'+options[k].option_id).append(
                                new_hyp.compose({
                                    'question_id' : rs[i].question_id,
                                    'option_id'  : options[k].option_id,
                                    'hyp_id' : option_hyps[h].hyp_id,
                                    'point' :  (option_hyps[h].strength == 1000 || option_hyps[h].strength == -1 || option_hyps[h].strength == null)?  "" : option_hyps[h].option_hyp_point,
                                    'option_hyp_id' : option_hyps[h].option_hyp_id,
                                    'hyp_content' : option_hyps[h].hyp_content,
                                    'label_m_100' : (hyp[0].label_m_100 == null) ? "":hyp[0].label_m_100,
                                    'label_m_50' : (hyp[0].label_m_50 == null) ? "" : hyp[0].label_m_50,
                                    'label_0' : (hyp[0].label_0 == null) ? "" : hyp[0].label_0,
                                    'label_p_50' : (hyp[0].label_p_50 == null) ? "" : hyp[0].label_p_50,
                                    'label_p_100' : (hyp[0].label_p_100 == null) ? "" : hyp[0].label_p_100,
                                    'strength' : option_hyps[h].strength
                                })
                            );


                            if(parseInt(option_hyps[h].strength) == 1000){
                                $('p[option_hyp_id="' + option_hyps[h].option_hyp_id + '"').parent().addClass('hide_zero_powers');
                                is_expand_bt_added = true;
                                count_zero_powers++;
                            }


                            if(option_hyps[h].strength == null || option_hyps[h].strength == -1){
                                $('#hypo_box_' + rs[i].question_id + '_' + options[k].option_id + '_' + option_hyps[h].hyp_id).children().eq(2).addClass('option-content-alert');
                            }
                        }
                ...
                if(is_expand_bt_added){
                    $('#question_block_' + rs[i].question_id).append(
                        question_expand_shrink_button.compose({
    ...
    $(document).on('click', '.bt-expand-shrink', function(e){
        ...
        $question_id = $(this).parent().attr('question_id');
        $counts    = $(this).parent().attr('counts');
        $content   = $(this).text();
       
        if($content.includes('expand')){
            $('div[question_id="' + $question_id + '"]').children('div').removeClass('hide_zero_powers');
            $(this).html('Shrink NONE');
        }
        else{
            $(this).html('...plus ' + $counts + ' other options with NONE HYPOTHESIS  [expand]');

            ($('div[question_id="' + $question_id + '"]').children('div.hypo-box')).each(function(i,item){
                if(parseInt($(item).attr('strength')) == 1000){
    ...
    $('#bt_all_zero').click(function(e){
        ...
        $(".option-point").each(function (index, element) {
            if($(this).val() == ""){
                $(this).val(0);
                $(this).next().next().removeClass('option-content-alert');
            }
        });
    ...
    /**
     * ------------------------------------
     * Add New question click handler
    */

    $('#add_new_question').click(function(e){
        new_question_id++; // question_id value of newly created question
        ...
        $('#question_block_' + new_question_id).find('input').focus(); 
        // set focus on new added question textbox
        
    });

     
    $(document).on('change', 'input.question-content', function(e){
    ...
    $(document).on('click', '.remove-question', function(e){
        ...
        $remove_question_id = $(e.target).parent().parent().parent().attr('question_id');
        $object_delete = $(e.target).parent().parent().parent();
        $trash_object = $(e.target);

        $question_content = $(e.target).parent().parent().find('.question-content').val();

        $('.delete-question').html("<span style='color:red;'>Do you really want to delete question : </span><br/>" + $question_content);
    ...
    $('#kt_confirm_delete .btn-delete').click(function(e) {
        $.post('/getdatainjson/deletequestion',{
                question_id   : $remove_question_id,
            }, function(res,status){
                if(status == 'success'){
    ...
    $(document).on('click','.add-option', function(e){
        ...
        $.post('/getdatainjson/add_empty_option_hyps', {scope_id : $scope_id,option_id : new_option_id}, function(rs){
            $.post('/getdatainjson/hyps', {scope_id : $scope_id}, function(result){
                ...
                $('#option_box_' + $question_id + '_' + new_option_id).find('input').focus(); 
                // set focus on new added option textbox

                var rs = JSON.parse(result);

                for(var h in rs){
                    $('#option_box_'+ $question_id + '_' + new_option_id).append(
                        new_hyp.compose({
                            'option_id'  : new_option_id,
                            'hyp_id' : rs[h].hyp_id,
                            'hyp_content' : rs[h].hyp_content,
                            'question_id' : $question_id,
                        })
                    );

                    $('#hypo_box_'  + $question_id + '_' + new_option_id + '_' + rs[h].hyp_id).children().eq(2).addClass('option-content-alert');
     ...
     */
    $(document).on('change', 'input.option-content', function(ev){
    ...
    /**
     * Once clicked "remove option" button next to "Link +",
     * Option block will be removed from Question Tree and its data will be delete from db.
     */

    $remove_option_id = -1;
    $option_object_delete = null;
    $option_trash_object = null;

    $(document).on('click', '.remove-option', function(e){

        $remove_option_id = $(e.target).parent().parent().attr('option_id');
        $option_object_delete = $(e.target).parent().parent();
        $option_trash_object = $(e.target);

        $option_content = $(e.target).parent().parent().find('.option-content').val();

        $('.option-delete-question').html("<span style='color:red;'>Do you really want to delete option : </span><br/>" + $option_content);
        $(e.target).addClass('rotate-trash');
        $('#option_confirm_delete').modal('show');

    });   


    $('#option_confirm_delete .btn-delete').click(function(e) {
        $.post('/getdatainjson/deleteoption',{
                option_id   : $remove_option_id,
            }, function(res,status){
                if(status == 'success'){
                    $option_object_delete.remove(); // Combo will be removed from UI.
                    $('#option_confirm_delete').modal('toggle');
                }
        }); 
    });

    $('#option_confirm_delete .btn-cancel').click(function(e){
        $option_trash_object.removeClass('rotate-trash');
    }) 
    ...
    $(document).on('click', '.add-hyp', function(e){
        ...
        new_hyp_id++; // This increases current maximum hyp id by one.
        new_option_hyp_id++; // This increases current maximum option_hyp id by one.

        $question_id = $(e.target).parent().attr('question_id');
        $option_id   = $(e.target).parent().attr('option_id');

        // This creates hyp select list with link textbox and then adds it to container
        ...
        $.post('/getdatainjson/allquestions', function(result){
            var rs = JSON.parse(result);
            // Hyps are added to "select box" newly created and hyps come from Hyp page
            for(var h in rs[0][0]){ 
            // This is bad (rs[0][0]) because current API is not efficient for Object array creating about questions
                $(e.target).parent().find('select').append(new Option(rs[0][0][h].hyp_content, rs[0][0][h].hyp_id));
            }
        });
    ...
    $(document).on('change', 'input.option-point', function(e){
        ...
        $.post('/getdatainjson/add_option_hyp',{
                ...
                if(status == 'success'){
                    setTimeout(function() {
                      $(e.target).removeClass('input_ajax_started').addClass('green_tick_started'); 
                      $(e.target).next().next().removeClass('option-content-alert');
                      // ajax waiting effect ended after 1s
                    }, 1000);
    ...
    /**
     * This handler is for limiting typing to only numbers regards on option-point 
     * This code is perfect, I think
     */ 
    $(document).on('keypress', 'input.option-point', function(e){
    ...
    /**
     * Each time hyp list changed, this handler is being called.
     * This results in link table update.
     */

    $(document).on('change', 'select.hypo-list', function(e){
        $hyp_id    = $(e.target).val(); 
        $option_id = $(e.target).parent().parent().attr('option_id');

        $option_hyp_id = $(e.target).prev().prev().attr('option_hyp_id');
        $option_hyp_point = $(e.target).prev().prev().val();
        
        $(e.target).addClass('input_ajax_started'); // ajax waiting effect started.

        if(($option_hyp_point != null) && ($option_hyp_point != "")){
            $.post('/getdatainjson/add_option_hyp',{
                option_hyp_id: $option_hyp_id, 
                option_id :  $option_id,
                hyp_id : $hyp_id,
    ...
    $('.scopes-select').change(function(e){
        ...
        is_order_button_clicked = true;
        $('#bt_change_order').html('&nbsp;CHANGE ORDER');
        $('.reorder-icon').hide(); // hide drag-icons
        $('.option-box').show();

        // ajax waiting effect started.
        $('.lds-dual-ring').css('visibility','visible');

        $.post('/getdatainjson/allquestions', {scope_id : $scope_id}, function(result){
            ...
            for(var i in rs) {
                ...
                for(var k in options){
                    ...
                    for(var h in option_hyps){
                        ...
                        if(option_hyps[h].scope_id == $('.scopes-select').val()){
                            $('#option_box_'+rs[i].question_id+'_'+k).append(
                                new_hyp.compose({
                                    'question_id' : rs[i].question_id,
                                    'option_id'  : k,
                                    'hyp_id' : option_hyps[h].hyp_id,
                            ...
                            if(option_hyps[h].strength == null || option_hyps[h].strength == -1 ){
                                $('#hypo_box_' + rs[i].question_id + '_' + k + '_' + option_hyps[h].hyp_id).children().eq(2).addClass('option-content-alert');
                            }
                        }
    ...
    $('#bt_change_order').click(function(e){
        if(is_order_button_clicked){
            $(this).html('&nbsp;SHOW H&O');
            
            $('.option-box').hide();
            $('.question-expand-shrink-button-block').hide();
            $('.reorder-icon').show(); // show drag-icons

            is_order_button_clicked = false;

            $( ".question-container" ).sortable({
                update: function() {
                    
                    $display_orders = [];
                    $('.question-container').find('.question-block').each(function(index, item){
                        $display_orders.push({'display_order' : index, 'question_id' : $(item).attr('question_id')})
                    });

                    $.post('/getdatainjson/save_display_oder', {display_orders : JSON.stringify($display_orders)},function(rs){
                        console.log(rs)
                    });
                }
            });
            ...
            $('.reorder-icon').hide(); // hide drag-icons
            $('.option-box').show();
            $('.question-expand-shrink-button-block').show();
        }
