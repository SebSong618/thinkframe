    ...
    <div class="d-flex align-items-stretch" id="kt_header_nav">
        ...
        <div class="header-menu align-items-stretch" data-kt-drawer="true" data-kt-drawer-name="header-menu" data-kt-drawer-activate="{default: true, lg: false}" data-kt-drawer-overlay="true" data-kt-drawer-width="{default:'200px', '300px': '250px'}" data-kt-drawer-direction="end" data-kt-drawer-toggle="#kt_header_menu_mobile_toggle" data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_body', lg: '#kt_header_nav'}">
            ...
            <div class="menu menu-lg-rounded menu-column menu-lg-row menu-state-bg menu-title-gray-700 menu-state-title-primary menu-state-icon-primary menu-state-bullet-primary menu-arrow-gray-400 fw-bold my-5 my-lg-0 align-items-stretch" id="#kt_header_menu" data-kt-menu="true">
                <span class="title-heading combo-title">Combos</span>
...
<div class="post d-flex flex-column-fluid" id="kt_post">
    ...
    <div id="kt_content_container" class="container">
        ...
        <div class="card combo-page">
         <!--    <a href="#" data-bs-toggle="modal" data-bs-target="#kt_confirm_delete">Share &amp; Earn</a> -->
            <!--begin::Card body-->
...
<script>
    ...
    /**
     * Get current max combo id from API.
     * Why this max combo_id is needed? It is for creating new combo. (usually new combo id = current max combo id + 1)
     */ 
    $.post('/getdatainjson/combo_max_id', function(result) {
        new_combo_id = parseInt(result);
    });

    // api call for getting combos list

    $.post('/getdatainjson/combos', {scope_id : $('.scopes-select').val()},function(result) {
        var rs = JSON.parse(result);
        for(var i in rs){

            $('.combo-container').append(
                combo_block.compose({
                    'combo_id'   : parseInt(rs[i].combo_id),
                    'combo_name' : rs[i].combo_name
                })
            );

             $('#combo_block_' + rs[i].combo_id).append(
                save_cancel_combo.compose({
                    'combo_id' : rs[i].combo_id
                })
            );
            ...
            for(var h in hyps){
                $('#combo_block_' + rs[i].combo_id).append(
                    hyp_list_item.compose({
                        ...
                        'power' : (hyps[h].power == null)? "" : hyps[h].power,
                        'power_id' : hyps[h].power_id,
                        'range_show' : (hyps[h].power == null)? 'hidden' : 'visible', 
                    })
                );

                if(parseInt(hyps[h].power) == 0){
                    $('#' + hyps[h].power_id).parent().addClass('hide_zero_powers');
                    is_expand_bt_added = true;
                    count_zero_powers++;
                }

                $last_saved_state.push({
                    'power_id' : parseInt(hyps[h].power_id),
                    'power'    : (hyps[h].power == null)? "" : hyps[h].power
                });

                // This means unlinked relationship between hyp and combo.
                if(hyps[h].power == null){ 
                    $('#hyp_list_item_'+rs[i].combo_id+'_'+hyps[h].hyp_id).find('.hyp-content').addClass('option-content-alert');
                }
            }
           
            if(is_expand_bt_added){
                $('#combo_block_' + rs[i].combo_id).append(
                    combo_expand_shrink_button.compose({
    ...
    $(document).on('click', '.bt-expand-shrink', function(e){
        ...
        $combo_id = $(this).parent().attr('combo_id');
        $counts    = $(this).parent().attr('counts');
        $content   = $(this).text();
       
        if($content.includes('expand')){
            $('div[combo_id="' + $combo_id + '"]').children('div').removeClass('hide_zero_powers');
            $(this).html('Shrink 0 powers');
        }
        else{
            $(this).html('...plus ' + $counts + ' other options with power 0  [expand]');

            ($('div[combo_id="' + $combo_id + '"]').children('div.hyp-list-item')).each(function(i,item){
    ...
    $('#add_new_combo').click(function(){
        ...
        $.post('/getdatainjson/hyps', {scope_id : $scope_id}, function(result){
            ...
            $('#combo_block_' + new_combo_id).find('input').first().focus(); // set focus on new added combo textbox
            ...
            for(var i in rs){
                // I think this is not good way to generate power id although it is random number.
                var $power_id = getRandomInt(10000,100000);
                ...
                var power = {combo_id : new_combo_id, hyp_id :  parseInt(rs[i].hyp_id), power_id: $power_id};
                    powers_notyet.push(power);

                $("div#hyp_list_item_" + new_combo_id +"_" +parseInt(rs[i].hyp_id) ).children().eq(3).addClass('option-content-alert');

            }

            $.post('/getdatainjson/add_powers_pending', {powers : JSON.stringify(powers_notyet)}, function(e){});
    ...
    $('#bt_all_zero').click(function(e){
        ...
        $(".combo-power").each(function (index, element) {
            if($(this).val() == ""){
                $(this).val(0);
                powers.push({'power_id' : $(this).attr('id'), 'hyp_id' : $(this).parent().attr('hyp_id'), 'combo_id' : $(this).parent().parent().attr('combo_id') });
                $(this).next().next().next().removeClass('option-content-alert');
            }
        });

        $.post('/getdatainjson/set_zeros_on_blanks', {table_selection : "powers", powers : JSON.stringify(powers)}, function(rs){
            $('.lds-dual-ring').css('visibility','hidden');
        });
    });

    /**
     * Combo delete handler.
     * This handler is for deleting Combo with corresponding combo id.
     */
    $combo_id = -1;
    $object_delete = null;
    $trash_object = null;
    ...
    $(document).on('click', '.remove-combo', function(e){
        $combo_id = $(e.target).parent().parent().parent().attr('combo_id');
        $object_delete = $(e.target).parent().parent().parent();
        $trash_object = $(e.target);

        $combo_name = $(e.target).parent().parent().find('.combo-name').val();

        $('.delete-question').html("<span style='color:red;'>Do you really want to delete combo : </span><br/>" + $combo_name + "?");
    ...
    /**
     * Each time combo content changed, 
    ...
    $(document).on('click', 'input.combo-power', function(e){
        if($(e.target).val() == ""){
    ...
    function normalization_by_power_input(power, $combo_id,$power_id, $rate){

        $sum = 0;
        // get total sum up.
        $('#combo_block_' + $combo_id).find('.combo-power').each(function(){
            if($(this).val() != ""){
    ...
    /**
     * Each time combo power content changed, 
     * its content will be added or updated on its matched table in db.
     */
    ...
    $(document).on('change', 'input.combo-power', function(e){
        ...
        normalization_by_power_input($(this).val(), $combo_id, $power_id, $rate);
        ...
        // Since combo power has value, red border line of hyp will be removed.
        ...
        $.post('/getdatainjson/add_power',{
                ...
                if(status == 'success'){
                    $('#combo_block_' + $combo_id).find('.save-cancel-block').show();
                }
        });
    ...
    $(document).on('input', 'input[type=range]', function(ev){
        ...
        $(ev.target).parent().find('input.combo-power').val($currentVal);
        $combo_id = $(ev.target).parent().parent().attr('combo_id');
        $powerid_on_change = $(ev.target).prev().attr('id');
        ...
        if($previous_value != 100){
            ...
            var hyp_length = $('#combo_block_' + $combo_id).find('.combo-power').filter(function(index){
                return this.value !== "";
            }).length;
            ...
            $('#combo_block_' + $combo_id).find('.combo-power').each(function(){
                ...
                if($(this).val() != ""){
                    if($(this).attr('id') != $powerid_on_change){
                        $common_value = (100 - $currentVal) / (hyp_length - 1);
    ...
    $(document).on('click', '.bt-save-normalization', function(e){
        ...
        $('#combo_block_' + $combo_id).find('.combo-power').each(function(){
            ...
            if($(this).val() != ""){
                $power = {
                    power_id : $(this).attr('id'),
                    power : parseInt($(this).val()),
                    combo_id : $combo_id,
                    hyp_id : parseInt($(this).parent().attr('hyp_id'))
                };

                // save state values to cancel later
                $index_found = $last_saved_state.findIndex(node => node.power_id === parseInt($(this).attr('id')));
                
                if($index_found > -1){
                   $last_saved_state[$index_found]['power'] = $(this).val();
                }

                powers_array.push($power);
            }
            
        });
        
        $.post('/getdatainjson/update_powers', {normalization_array : JSON.stringify(powers_array)}, function(res){
            console.log(res)
            $(e.target).removeClass('input_ajax_started').addClass('green_tick_started');
                setTimeout(function() {
                    $(e.target).removeClass('green_tick_started'); // ajax waiting effect ended
    ...
    $(document).on('click', '.bt-cancel-normalization', function(e){
        ...
        var ranges = $('#combo_block_' + $combo_id).find('input[type=range]');
       
        $('#combo_block_' + $combo_id).find('.combo-power').each(function(i){
            $index_found = $last_saved_state.findIndex(node => node.power_id === parseInt($(this).attr('id')));
            
            if($index_found > -1){
                $(this).val($last_saved_state[$index_found]['power']);
                $(ranges[i]).val($last_saved_state[$index_found]['power']);
            }
        });
    });
    ...
    $('.scopes-select').change(function(e){
        ...
        $.post('/getdatainjson/combos',{scope_id : $scope_id}, function(result) {
            var rs = JSON.parse(result);
